...
//      std::uniform_int_distribution<> distr( -0.5*1000, 0.5*1000); // define the range
...
static void derSigmoid( double& derivative, double output, double specificOutput, double numberOf, vector<double>& list){
...
static collectFuncDerivative derivativeFunc[7]={&derAddition, &derReLu, &derMultiply, &derAverage, &derSubtraction, &derSigmoid, &derTanh};
